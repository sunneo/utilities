...
namespace FileLockInfo
{
    public class Win32Processes
    {
        ...
        public static List<Process> GetProcessesLockingFile(string filePath,bool foundOneBreak=false)
        {
            ...
            if (!foundOneBreak)
            {
                foreach (var process in processListSnapshot)
                {
                    ...
                    if (files.Contains(filePath))
                    {
                        ...
                        Console.WriteLine("--------------->" + process.ProcessName);
                        procs.Add(process);
                        if (foundOneBreak) break;
                    }
                }
            }
            else 
            {
                Utilities.Parallelx.For(0,processListSnapshot.Length,(int idx, CancellationTokenSource c, int id) =>
                {
                    Process process = processListSnapshot[idx];
                    Console.WriteLine(process.ProcessName);
                    if (process.Id <= 4) { return; } // system processes
                    var files = GetFilesLockedBy(process);
                    if (files.Contains(filePath))
                    {
                        Console.WriteLine("--------------->" + process.ProcessName);
                ...
                }, true, Utilities.Parallelx.ParallelForScheduler.RuntimeLoadBalance);
            }
            return procs;
        }
